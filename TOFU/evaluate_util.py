...
def get_all_evals(cfg, model, tokenizer, eval_task, eval_dataloader, base_eval_dataloader, perturb_dataloader, normalize_gt=False):
    ...
    for batch in tqdm(eval_dataloader):
        ...
        # Debug: Check label values before forward pass
        valid_labels = batch['labels'][batch['labels'] != -100]
        if len(valid_labels) > 0:
            max_label = valid_labels.max().item()
            model_vocab = model.config.vocab_size
            if max_label >= model_vocab:
                print(f"ERROR: Max label {max_label} >= model vocab size {model_vocab}")
                print(f"   Batch indices: {indices}")
                print(f"   Labels shape: {batch['labels'].shape}")
                raise ValueError(f"Label {max_label} out of range for vocab size {model_vocab}")
...
def main(cfg):
    ...
    # Try to load tokenizer from model path first, fallback to base model
    if not cfg.use_pretrained:
        try:
            print(f"Trying to load tokenizer from {cfg.model_path}")
            tokenizer = AutoTokenizer.from_pretrained(cfg.model_path)
            print(f"Successfully loaded tokenizer from model path")
        except:
            print(f"Failed to load tokenizer from model path, using base model tokenizer from {model_id}")
    ...
    # Use attn_implementation instead of use_flash_attention_2 (deprecated)
    use_flash_attn = model_cfg["flash_attention2"] == "true"
    ...
    for attempt in range(3):
        try:
            if cfg.use_pretrained:
                ...
                # Remove deprecated use_flash_attention_2 from config
                if hasattr(config, 'use_flash_attention_2'):
                    delattr(config, 'use_flash_attention_2')
                if hasattr(config, '_flash_attn_2_enabled'):
                    delattr(config, '_flash_attn_2_enabled')

                # Override attn_implementation in config to avoid FlashAttention2
                ...
                model = AutoModelForCausalLM.from_pretrained(
                    cfg.model_path,
                    config=config,
                    attn_implementation="eager",  # Force eager for checkpoints
                    torch_dtype=torch.bfloat16,
                    trust_remote_code=True,
                    device_map=device_map,
                    local_files_only=False  # Allow loading from HuggingFace Hub
                )
    ...
    if model is None:
        print("Error: could not load model after 3 attempts")
        raise RuntimeError("Failed to load model")

    # Check tokenizer/model vocab size mismatch
    tokenizer_vocab_size = len(tokenizer)
    model_vocab_size = model.config.vocab_size
    if tokenizer_vocab_size != model_vocab_size:
        print(f"WARNING: Tokenizer vocab size ({tokenizer_vocab_size}) != Model vocab size ({model_vocab_size})")
        print(f"   This is normal for Qwen models - they have reserved tokens in the embedding layer")
        print(f"   Resizing model embeddings to match tokenizer...")
        model.resize_token_embeddings(len(tokenizer))
        print(f"   Model embedding size is now: {model.get_input_embeddings().weight.shape[0]}")
...
def run_generation(cfg, batch, model, tokenizer):
    ...
    if cfg.model_family == 'llama2-7b':
        split_symbol = " [/INST]"
    elif cfg.model_family == 'llama3-8b':
        # Llama 3: after skip_special_tokens=True, format becomes "user\n\nQuestion...assistant\n\nAnswer"
        split_symbol = "assistant\n\n"
    elif cfg.model_family.startswith('qwen') or cfg.model_family.startswith('ministral'):
        # For models using chat templates (Qwen, Ministral, etc.)
        # Format: <|im_start|>assistant\nAnswer<|im_end|>
        split_symbol = "assistant\n"
    else:
        # For phi, stablelm, pythia models
        split_symbol = 'Answer: '

    # Split to extract ground truth and input
    ground_truth = []
    input_only = []
    for s in input_strings:
        parts = s.split(split_symbol)
        if len(parts) < 2:
            # If split fails, use empty string for ground truth
            print(f"Warning: Could not split string with symbol '{split_symbol}'. String: {s[:100]}...")
            ground_truth.append("")
            input_only.append(s)
        else:
            ground_truth.append(parts[1])
            input_only.append(parts[0])

    input_strings = input_only

    #add split symbol back to the end of each string for generation prompt
    if cfg.model_family == 'llama2-7b':
        input_strings = [s + split_symbol for s in input_strings]
    elif cfg.model_family == 'llama3-8b':
        # For Llama 3, add the assistant header back
        input_strings = [s + split_symbol for s in input_strings]
    elif cfg.model_family.startswith('qwen') or cfg.model_family.startswith('ministral'):
        # For chat template models, add the assistant prompt
        input_strings = [s + split_symbol for s in input_strings]
