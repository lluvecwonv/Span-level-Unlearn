...
# python sc.py --file sc_experiment --name v.1


def load_model(model_name):
    print(f"Loading model: {model_name}")
    # Load tokenizer from base Llama 2 model since fine-tuned model may not have tokenizer files
...
def generate_batch_responses(tokenizer, model, prompts, max_new_tokens, temperature):
    ...
    For LLaMA 2, prompts are already formatted with special tokens.
    """

    # 1) For LLaMA 2, use the full prompt as-is (already formatted with special tokens)
...
def extract_high_si_score_word(word_file,text_file,threshold):
    ...
    # MUSE news data is stored as a JSON array, so handle it separately.
    with open(text_file, "r", encoding = "utf-8") as f:
        data = json.load(f)

    for idx in result:
        if idx < len(data):
            # MUSE news data is a simple string array.
            result[idx]["text"] = data[idx]
...
def process_sample_with_self_consistency(tokenizer, model, sample_index, high_si_score, text, all_words, iteration):
    ...
    system_msg = """You are a high-precision data forensics agent specializing in Machine Learning Unlearning. Your goal is to identify the "Forget Set"—specific text segments within a document that contribute to a high Self-Influence (SI) score.
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Run MUSE span selection with self-consistency')
    ...
    # File paths (updated for MUSE environment)
    word_file = os.path.join(muse_root, "self_conf", "llama3.json")  # SI score file.
    text_file = os.path.join(muse_root, "data", "news", "raw", "forget.json")  # MUSE news data.
    gt_csv_file = os.path.join(muse_root, "self_conf", "gt_token_llama3.csv")  # Ground-truth file.
    ...
    if args.name:
        ...
        output_filename = "result_ours_llama2.json"
    ...
    # Use the MUSE news fine-tuned Llama 2 model.
